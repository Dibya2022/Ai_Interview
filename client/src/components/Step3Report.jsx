...
function Step3Report({ report }) {
  ...
  const topStrength = [...skills].sort((a, b) => b.value - a.value)[0];
  const focusArea = [...skills].sort((a, b) => a.value - b.value)[0];
  ...
  const actionPlan = [
    confidence < 7
      ? "Record a 60-second self-introduction daily to build speaking confidence."
      : "Keep your speaking pace steady and maintain concise opening answers.",
    communication < 7
      ? "Use STAR structure for each answer: Situation, Task, Action, Result."
      : "Focus on sharper examples to make your answers more memorable.",
    correctness < 7
      ? "Revise core concepts and explain each one with a real project example."
      : "Add trade-offs and edge cases when answering technical questions."
  ];
  ...
  const copySummary = async () => {
    const summaryText = `Interview Score: ${finalScore}/10 | Confidence: ${confidence}/10 | Communication: ${communication}/10 | Correctness: ${correctness}/10 | Strength: ${topStrength.label} | Focus: ${focusArea.label}`
    ...
    <div className='min-h-screen px-4 sm:px-6 lg:px-10 py-8'>
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        ...
        <div className='flex gap-2'>
          <button onClick={copySummary} className='secondary-btn px-5 py-3 text-sm sm:text-base'>
        ...
        <div className='space-y-6'>
          ...
          <motion.div
            ...
            <div className='space-y-3'>
              <div className='rounded-xl border border-[#d6e8e1] bg-white px-4 py-3 text-sm text-slate-700'>
                Top Strength: <span className='font-semibold text-[#0f7f67]'>{topStrength.label}</span> ({topStrength.value}/10)
              </div>
              <div className='rounded-xl border border-[#d6e8e1] bg-white px-4 py-3 text-sm text-slate-700'>
                Focus Area: <span className='font-semibold text-[#0f7f67]'>{focusArea.label}</span> ({focusArea.value}/10)
              </div>
            </div>
          </motion.div>
        ...
        <div className='lg:col-span-2 space-y-6'>
          ...
          <motion.div
            ...
            <h3 className="text-base sm:text-lg font-semibold text-gray-700 mb-5">
              7-Day Improvement Action Plan
            </h3>

            <div className='space-y-3'>
              {actionPlan.map((item, index) => (
                <div key={index} className='rounded-xl border border-[#d6e8e1] bg-white px-4 py-3 text-sm text-slate-700'>
                  Day {index + 1}: {item}
                </div>
              ))}
            </div>
